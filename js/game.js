...
var Game = {
    ...
    create: function() {
        ...
        // The player's life
        player_life_text = game.add.text(650, 16, 'Life: 100', { fontSize: '32px', fill: '#000' });
    ...
    update: function() {
    	...
    	game.physics.arcade.overlap(player, firstaid, this.speed_bonus, null, this);

        //Check if the player has the gravity potion
        game.physics.arcade.overlap(player, diamond, this.gravity_bonus, null, this);

        // Check if the player or the mob touches each other
        game.physics.arcade.overlap(player, mob, this.hurt_player, null, this);

        this.move_the_player();
    ...
    speed_bonus: function(player, firstaid) {
    	firstaid.kill();
    ...
    gravity_bonus : function(player, diamond) {
        diamond.kill();
    ...
    move_the_mob : function() {
        ...
        if (move === 1){
            ...
            this.mob_bounce();
            if(!mob.body.touching.down && mob.body.velocity.y < 0 ){
                mob.body.velocity.y = -250
            }
    ...
    hurt_player : function(player,mob){
